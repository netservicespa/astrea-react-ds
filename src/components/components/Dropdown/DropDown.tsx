...
export const DynamicLink = ({ router, to, children }: DynamicLinkProps) => {
	...
	if (isReactRouter) {
		return (
			<a onClick={() => router.history.push(to)} className={'font-semiBold'} style={{ textDecoration: 'none', cursor: 'pointer' }}>
				{children}
			</a>
		);
	} else if (isNextRouter) {
		return (
			<a onClick={() => router.push(to)} className={'font-semiBold'} style={{ textDecoration: 'none', cursor: 'pointer' }}>
				{children}
			</a>
		);
	} else {
		...
		return (
			<a href={to} className={'font-semiBold'} style={{ textDecoration: 'none' }}>
				{children}
			</a>
		);
	}
...
export const DropdownComponent = ({router,dropdownItems,onLogout,children,dropDownConfiguration}:IDropDown) => {
	...
	return (
		<>
			<div style={{cursor:'pointer'}} onClick={handleMenuOpen}>{children}</div>
			...
			<React.Fragment>
				...
				>
					...
					<MenuItem onClick={() => handleMenuItemClick("Logout")}>
						...
						<ListItemText primary="Logout" style={{ color: '#FFF' }} onClick={onLogout}/>
					</MenuItem>
				</Menu>
			</React.Fragment>
		</>
