...
function CustomTabPanel({ children, value, index, noPadding, ...other }: TabPanelProps) {
...
export const NsTabs: React.FC<NsTabsProps & Omit<TabsProps, 'value' | 'onChange' | 'children'>> = ({
    tabs,
    configuration,
    ...otherProps
}) => {
    ...
    return (
        ...
        >
            ...
            {tabs.map((tab, index) => (
                <CustomTabPanel key={tab.id} value={value} index={index} noPadding={tab.noPadding}>
                    {tab.children}
                </CustomTabPanel>
            ))}
        </Box>
