...
import { ValidateFunction } from 'relay-forms';

export type ValidatorType<ValueType = unknown> = ValidateFunction<ValueType>;

export type ValidatorFactoryType<ValueType = unknown> = (message: React.ReactNode) => ValidatorType<ValueType>;

export type NsInput<InputType, ValueType> = InputType & {
    /** Funzione o array di funzioni per la validazione */
    validate?: ValidatorFactoryType<ValueType> | ValidatorFactoryType<ValueType>[];
    /** Messaggio di errore */
...
export const required: ValidatorFactoryType =
    ...
    (v: ValueType) =>
        (!v || (Array.isArray(v) && (v as any).length === 0) ? message : undefined) as string;

export const composeValidators =
    <ValueType>(
        validators?: ValidatorFactoryType<ValueType> | ValidatorFactoryType<ValueType>[],
